 ...
 * @author pruebahadoop
 */
public class Mapper_superficiexProductoYCampaña extends Mapper<LongWritable, Text, ProductoCampañaWritable, DoubleWritable> {
	...
	public void map(LongWritable ikey, Text ivalue, Context context) throws IOException, InterruptedException {
		...
		ProductoCampañaWritable campañaProducto = new ProductoCampañaWritable(cr.getCelda().getProdCamp().getProducto().toString(), cr.getCelda().getProdCamp().getCampaña().toString());
		OperadorCelda oc = new OperadorCelda(cr.getCelda(), anchoCelda);
		DoubleWritable sup = new DoubleWritable(oc.getSuperficie());

		context.write(campañaProducto, sup);
	}
