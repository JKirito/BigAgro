...
public class CeldaWritable implements WritableComparable<CeldaWritable> {
	...
	public CeldaWritable() {
		...
		prodCamp = new ProductoCampañaWritable();
	}

	public CeldaWritable(Integer fila, Integer columna, String producto, String campaña) {
		super();
		this.fila = new IntWritable(fila);
		this.columna = new IntWritable(columna);
	...
	public String toString() {
		return prodCamp.toString() + "," + columna + "," + fila;
	}

	@Override
	public int compareTo(CeldaWritable cw) {
		int cmp = this.prodCamp.compareTo(cw.prodCamp);

		if (cmp != 0) {
			return cmp;
		}
		cmp = columna.compareTo(cw.columna);
		if (cmp != 0) {
			return cmp;
		}
		return fila.compareTo(cw.fila);
	}

	// public static class Comparator extends WritableComparator {
	//
	// private static final IntWritable.Comparator INT_COMPARATOR = new
